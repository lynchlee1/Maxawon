...
## Project Notes: Maxawon

These notes capture the current product direction without implementing it yet.

### Compliance Boundary

- Do not implement or recommend bypasses for Maxawon's crawling prevention, bot detection, access controls, rate limits, CAPTCHA, fingerprinting, or other anti-automation measures.
...
- The program should search Maxawon using the available identifiers and collect the matching record only when the match is unambiguous.
